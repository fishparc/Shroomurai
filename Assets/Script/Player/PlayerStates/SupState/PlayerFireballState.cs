...
public class PlayerFireballState : PlayerAbilityState
{
    ...
    public override void Enter()
    {
        ...
        player.FireballDirectionIndicator.gameObject.SetActive(true);
    ...
    public override void LogicUpdate()
    {
        ...
        if(!isExitingState)
        {
            if(isHolding)
            {
                ...
                fireballDirectionInput = player.inputHandler.FireballDirectionInput;
                ...
                if(fireballStopInput || Time.unscaledTime >= startTime + playerData.maxHoldTime)
                {
                    ...
                    player.FireballDirectionIndicator.gameObject.SetActive(false);
                    //trun
		        ...
		        player.FireballDirectionIndicator.rotation = rotation;
            }
            else
            {
                if(!fireballUsed)
                {
                    player.FireballScript.Fireball();
                    fireballUsed = true;
                }
                //執行丟火球
                //丟火球時間
    ...
    void RotateAimPivot()
	{
		Vector2 dir = player.inputHandler.RawFireballDirectionInput;
